...
public final class HatService {
    ...
    private final HatEquipmentService equipment;
    ...
    public ClickResult clickHat(Player player, String hatId) {
        var hat = catalog.find(hatId);
        if (hat.isEmpty()) {
            return ClickResult.UNKNOWN;
        }
        if (repository.ownsHat(player.getUniqueId(), hatId)) {
            equipment.equip(player, hatId);
            return ClickResult.EQUIPPED;
        }
        int price = repository.priceOf(hatId);
