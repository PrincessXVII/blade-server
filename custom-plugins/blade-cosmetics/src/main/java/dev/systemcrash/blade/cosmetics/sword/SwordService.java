...
public final class SwordService {
    ...
    private final SwordEquipmentService equipment;
    ...
    public SwordService(
            ...
            OrbsManager orbs,
            SwordEquipmentService equipment
    ) {
        this.plugin = plugin;
    ...
    public ClickResult clickSword(Player player, String swordId) {
        var sword = catalog.find(swordId);
        if (sword.isEmpty()) {
            return ClickResult.UNKNOWN;
        }
        if (repository.ownsSword(player.getUniqueId(), swordId)) {
            equipment.equip(player, swordId);
            return ClickResult.EQUIPPED;
        }
        int price = repository.swordPriceOf(swordId);
        if (!orbs.trySpend(player.getUniqueId(), price)) {
            sendNotEnough(player);
            player.playSound(player.getLocation(), Sound.ENTITY_VILLAGER_NO, 1f, 1f);
            return ClickResult.NOT_ENOUGH_ORBS;
        }
        repository.grantSword(player.getUniqueId(), swordId);
        equipment.equip(player, swordId);
        player.playSound(player.getLocation(), Sound.ENTITY_EXPERIENCE_ORB_PICKUP, 1f, 1.2f);
        return ClickResult.PURCHASED;
    }

    public void clearSword(Player player) {
